...
OFFENSIVE_RATE_CATEGORIES = [
    'Points Per Game', 'Points Per 36 Minutes', 'Points Per 100 Possessions',
    'Assists Per Game', 'Assists Per 36 Minutes', 'Assists Per 100 Possessions',
    'Field Goals Per Game', 'Field Goals Per 36 Minutes', 'Field Goals Per 100 Possessions',
    'Field Goal Attempts Per Game', 'Field Goal Attempts Per 36 Minutes', 'Field Goal Attempts Per 100 Possessions',
    'Three Pointers Per Game', 'Three Point Attempts Per Game',
    'Free Throws Per Game', 'Free Throws Per 36 Minutes', 'Free Throws Per 100 Possessions',
    'Free Throw Attempts Per Game', 'Free Throw Attempts Per 36 Minutes', 'Free Throw Attempts Per 100 Possessions',
    'Offensive Rebounds Per Game', 'Offensive Rebounds Per 36 Minutes', 'Offensive Rebounds Per 100 Possessions',
    'Field Goal Pct', 'Three Point Pct', 'Free Throw Pct', 
    'True Shooting Pct', 'Effective Field Goal Pct',
    'Offensive Rating', 'Offensive Box Plus/Minus',
    'Usage Pct', 'Player Efficiency Rating',
    'Win Shares Per 48 Minutes', 'Value Over Replacement Player', 'Box Plus/Minus',
    'Minutes Per Game', 'Turnovers Per Game', 'Turnovers Per 36 Minutes', 'Turnovers Per 100 Possessions', 'Turnover Pct',
    'Offensive Rebound Pct'
]

DEFENSIVE_RATE_CATEGORIES = [
    'Steals Per Game', 'Steals Per 36 Minutes', 'Steals Per 100 Possessions', 'Steal Pct',
    'Blocks Per Game', 'Blocks Per 36 Minutes', 'Blocks Per 100 Possessions', 'Block Pct',
    'Defensive Rebounds Per Game', 'Defensive Rebounds Per 36 Minutes', 'Defensive Rebounds Per 100 Possessions',
    'Defensive Rebound Pct',
    'Defensive Rating', 'Defensive Box Plus/Minus',
    'Personal Fouls Per 36 Minutes', 'Personal Fouls Per 100 Possessions',
    'Assist Pct'
]

EXCLUDE_CATEGORIES = [
    'Points', 'Assists', 'Field Goals', 'Field Goal Attempts',
    'Three Pointers', 'Three Point Attempts', 'Free Throws', 'Free Throw Attempts',
    'Offensive Rebounds', 'Defensive Rebounds', 'Total Rebounds',
    'Steals', 'Blocks', 'Turnovers', 'Personal Fouls',
    'Total Rebound Pct', 'Minutes Played', 'Games',
...
def analyze_team_rate(team_abbr, season='2025-26'):
    ...
    for _, row in team_data.iterrows():
        ...
        if rank <= 20:
            ...
            is_offensive = any(off_cat.lower() in category.lower() for off_cat in OFFENSIVE_RATE_CATEGORIES)
            is_defensive = any(def_cat.lower() in category.lower() for def_cat in DEFENSIVE_RATE_CATEGORIES)
...
def main():
    ...
    for team in teams:
        results[team] = analyze_team_rate(team)
        print(f"分析完成: {results[team]['team_name']}")
    
    print("\n=== 2025-26赛季 四队攻防占比对比（率指标版）===")
    print("分类规则：只保留Per Game/Per 36/Per 100 Possessions/Pct/Rating等率指标")
    print(f"\n{'球队':<15} {'进攻贡献':<12} {'防守贡献':<12} {'进攻占比':<8} {'防守占比':<8}")
    print("-" * 65)
    for team in teams:
        r = results[team]
    ...
    for team in teams:
        r = results[team]
        print(f"{r['team_name']}: 进攻{r['offensive_categories']}个 + 防守{r['defensive_categories']}个 = 共{r['offensive_categories']+r['defensive_categories']}个")
    ...
    ax1.set_title('四队攻防占比对比（率指标版）', fontsize=14, fontweight='bold')
    ...
    ax2.set_title('四队总贡献值对比（率指标版）', fontsize=14, fontweight='bold')
    ...
    plt.tight_layout()
    chart_file = OUTPUT_DIR / "four_teams_comparison_rate.png"
    plt.savefig(chart_file, dpi=150, bbox_inches='tight', facecolor='white')
    plt.close()
    
    print(f"\n📊 四队对比图表（率指标版）已保存到: {chart_file}")
